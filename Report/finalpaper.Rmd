---
title: "Student Grade Predictions for Online Learning"
...
During initial data exploration, a histogram was created to check whether the dependent variable (Class) followed a normal distribution pattern. There are more students observed in the middle than on either side indicating a normal distribution pattern. 
```{r plot 1}
hist(stuperform$Class)

```

correlation matrix to determine which variable was the most important in predicting the outcome (raised hands)
```{r plot 2}
