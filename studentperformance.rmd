...
Crab is a high-end food item consumed in large quantities every year. Due to its large demand, crab farms 
...
#add a column for ID
crabs <- mutate(crab, id = row_number())
...
for(i in categorical){
  crabs[,i] <- as.factor(crabs[,i])
}

for(i in prop){
  crabs[,i] <- ifelse(crabs[,i] == 0, .0001, crabs[,i])
  crabs[,i] <- ifelse(crabs[,i] == 1, .9999, crabs[,i])
}

crab_blueprint <- recipe(x = crabs,
                         vars = c(outcome, id, categorical, prop),
                         roles = c(rep('predictor', 8), 'outcome', 'id')) %>%
  step_indicate_na(all_of(categorical), all_of(prop)) %>%
  step_impute_mean(all_of(prop)) %>%
  step_impute_mode(all_of(categorical)) %>%
  step_logit(all_of(prop)) %>%
  step_ns(all_of(prop), deg_free = 3) %>%
  step_normalize(paste0(prop, '_ns_1'),
                 paste0(prop, '_ns_2'),
                 paste0(prop, '_ns_3')) %>%
